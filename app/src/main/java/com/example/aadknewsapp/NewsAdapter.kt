...
class NewsAdapter(val newsList: MutableList<Article>, val context: Context): RecyclerView.Adapter<NewsAdapter.NewsViewHolder>() {
    ...
    fun addItems(newArticles: List<Article>) {
        val previousSize = newsList.size
        newsList.addAll(newArticles)
        val newArticlesListSize = newArticles.size
        notifyItemRangeInserted(previousSize, newArticlesListSize)
